...
13. In crowsnest.conf change `mode` to `camera-streamer`.
14. Back in mainsail, edit your web camera settings and make sure the "Camera Streamer" option is selected.
15. Change `URL Stream` and `URL Snapshot` to `webcam/webrtc`
